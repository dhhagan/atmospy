...
**atmospy components used:** {components}
...
class ExampleGenerator:
    ...
    @property
    def plotfunc(self):
        match = re.search(r"atmospy\.(.+plot)\(", self.filetext)
        if match:
            return match.group(1)
        match = re.search(r"atmospy\.(.+map)\(", self.filetext)
        if match:
            return match.group(1)
        match = re.search(r"atmospy\.(.+Grid)\(", self.filetext)
        if match:
            return match.group(1)
        return ""
    ...
    def components(self):
        ...
        objects = re.findall(r"atmospy\.(\w+)\(", self.filetext)
